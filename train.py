...
def processFiles(pos_dir, neg_dir, recurse=False,output_file=False,
    ...
    if not (hog_features or hist_features or spatial_features):
        raise RuntimeError("No features selected (set hog_features=True,hist_features=True, and/or spatial_features=True.)")
    ...
    if recurse:
        pos_files = [os.path.join(rootdir, file) for rootdir, _, files in os.walk(pos_dir) for file in files]
        neg_files = [os.path.join(rootdir, file) for rootdir, _, files in os.walk(neg_dir) for file in files]
    else:
        pos_files = [os.path.join(pos_dir, file) for file in os.listdir(pos_dir) if os.path.isfile(os.path.join(pos_dir, file))]
        neg_files = [os.path.join(neg_dir, file) for file in os.listdir(neg_dir) if os.path.isfile(os.path.join(neg_dir, file))]
    ...
    img=cv2.imread(r"samples\vehicles\test3.jpg")
    i=cv2.cvtColor(img,cv2.COLOR_BGR2YCrCb)
    cv2.imshow("heloo",i)

    # Get names of desired features.
    features = [feature_name for feature_name, feature_bool in zip(["HOG", "color histogram", "spatial"],
    ...
    print("Converting images to " + color_space_name + " color space and "+ "extracting " + feature_str +
          " features from channel(s) "+ channel_index_str + ".\n")
...
def trainSVM(filepath=None,feature_data=None,C=1,
    ...
    # Safeguard: skip validation if empty
    if neg_val.shape[0] > 0:
        neg_val_predicted = classifier.predict(neg_val)
    else:
        neg_val_predicted = []

    if pos_val.shape[0] > 0:
    ...
    # Only perform hard negative mining if validation exists
    if neg_val.shape[0] > 0 and len(neg_val_predicted) > 0:
        neg_train = np.vstack((neg_train, neg_val[neg_val_predicted == 1, :]))

    if pos_val.shape[0] > 0 and len(pos_val_predicted) > 0:
        pos_train = np.vstack((pos_train, pos_val[pos_val_predicted != 1, :]))
    ...
    # Safeguard: skip test prediction if empty
    if pos_test.shape[0] > 0:
        pos_test_predicted = classifier.predict(pos_test)
    else:
        pos_test_predicted = []

    if neg_test.shape[0] > 0:
    ...
    if pos_test.shape[0] > 0 and neg_test.shape[0] > 0:
        ...
        print("Skipping test accuracy calculation (empty test set).")
